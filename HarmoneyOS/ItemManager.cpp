...
bool ItemManager::ShowPurchaseRecord() {
	...
	for (int i = 1; i <= MaxId; ++i) {
		myprintf("%d 月 订单编号： %d 该订单共消费： %lf 该订单共牟利： %lf \n",
...
bool ItemManager::ShowAllItemRecord() {
	...
	for (int i = 0; i < n; ++i) {
		...
		myprintf("物品编号： %s  ", Goods[i].Item.ItemId.c_str());
		myprintf(" 物品名称： %s  ", Goods[i].Item.ItemName.c_str());
		myprintf("该物品进价为 %lf 元，售价为 %lf 元，净利润为 %lf 元 ",
			Goods[i].Item.Cost, Goods[i].Item.Price, Goods[i].Item.Price - Goods[i].Item.Cost);
		if (Goods[i].Item.ItemType) myprintf("该物品以单个形式售卖, 剩余 %d 个\n", Goods[i].CountRest);
		else myprintf("该物品以散装称重形式售卖, 剩余 %lf 斤\n", Goods[i].WeightRest);
	}
	return 0;
}
